...
variable "public_subnets" {
  ...
  # cross-checks the union of isolated/private/public subnets, since each subnet's own
  # overlap validation above only compares CIDRs within its own map
  validation {
    condition = try(alltrue([
      for pair in setproduct(
        concat(
      ...
      ) : pair[0].key == pair[1].key || pair[0].start > pair[1].end || pair[1].start > pair[0].end
    ]), false)
    error_message = "Subnet CIDR blocks in isolated_subnets, private_subnets, and public_subnets must not overlap with one another."
  }
}
