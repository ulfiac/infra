...
resource "aws_glue_catalog_table" "cloudtrail_logs" {
  ...
  parameters = {
    ...
    "storage.location.template"    = "s3://${local.log_bucket_name}/${var.key_prefixes.cloudtrail}/AWSLogs/${local.aws_account_id}/CloudTrail/$${region}/$${day}",
  }
  ...
  storage_descriptor {
    ...
    location                  = "s3://${local.log_bucket_name}/${var.key_prefixes.cloudtrail}/AWSLogs/"
    number_of_buckets         = -1
